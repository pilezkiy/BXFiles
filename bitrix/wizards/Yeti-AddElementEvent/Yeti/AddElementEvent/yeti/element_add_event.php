<?
define("YETI_ADD_IBLOCKELEMENT_WIZ",true);

AddEventHandler("iblock", "OnAfterIBlockElementAdd", Array("CYetiElementAddWizard", "OnBeforeIBlockElementAddHandler"));

class CYetiElementAddWizard
{
    function OnBeforeIBlockElementAddHandler(&$arFields)
	{
		if($arFields['WF_NEW']) return true;
		...
		while ($arET = $rsET->Fetch())
		{
			...
			while($arSec = $sec->Fetch())
				$arSections[] = $arSec["NAME"];
			if(count($arSections) > 0)
				$fields["SECTIONS"] = implode(" / ",$arSections);
			...
			if($ar_res = $res->GetNext())
				$fields['DIRECT_LINK'] = 'http://'.$_SERVER['SERVER_NAME'].'/bitrix/admin/iblock_element_edit.php?ID='.$arFields['ID'].'&type='.$ar_res['IBLOCK_TYPE_ID'].'&IBLOCK_ID='.$arFields['IBLOCK_ID'];

			$ps = Array();
			$props = CIBlockElement::GetProperty($arFields['IBLOCK_ID'], $arFields['ID']);
			while($ar_props = $props->Fetch())
			{
				if(empty($ar_props['CODE']))
					$ar_props['CODE'] = $ar_props['ID'];
					
				if(empty($ps[$ar_props['CODE']]))
					$ps[$ar_props['CODE']] = Array();
			...
			foreach($ps as $code => $vals)
			{
				...
				{
					if($i) $fields["PROPERTY_".$code] .= '; ';
					$fields["PROPERTY_".$code] .= $val;
				}
			}
